...
export function ScoreRow({ color, row, availableNumbers, onMark, globallyLocked }: ScoreRowProps) {
    ...
    <div className={`score-row score-row-${color} ${isLocked ? 'locked' : ''}`}>
      {numbers.map((num) => {
        ...
        return (
          <button
            key={num}
            className={`score-cell ${isMarked ? 'marked' : ''} ${isAvailable ? 'available' : ''} ${isLastNumber ? 'last-number' : ''}`}
